...
| Plan document | `src/plan.rs`, `"plan_schema_version": 1`, schema at `docs/schema/plan-v1.schema.json` (#75) | Stable and additive-only, and byte-exact in a way no other surface here is: key order, whitespace and number formatting are frozen, not just field names and types. A plan file written by an earlier v1.x release remains loadable by a later v1.x binary — `Plan::from_canonical_bytes` refuses only a `plan_schema_version` newer than the build understands. Unknown top-level fields are ignored; unknown instruction variants (an `op.action`, `fingerprint.kind`, or `disposition` the build does not recognize) are refused, so a future executor never silently skips an operation it does not understand. The execution-boundary contract is path-only through `Plan::load_from_regular_file`, with symlinks and non-regular sources refused, followed by `Plan::verify` against current source and entry identities before action one. No production executor is part of this surface yet. See ADR 0006. |
